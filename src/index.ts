...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Giphy');
}
...
async function searchGifs(query: string, limit = 10) {
  ...
  const res = await pwFetch(`${BASE_URL}/search?${params}`);
  if (!res.ok) throw await httpError(res, 'Giphy API error');
...
async function trendingGifs(limit = 10) {
  ...
  const res = await pwFetch(`${BASE_URL}/trending?${params}`);
  if (!res.ok) throw await httpError(res, 'Giphy API error');
...
async function randomGif(tag?: string) {
  ...
  const res = await pwFetch(`${BASE_URL}/random?${params}`);
  if (!res.ok) throw await httpError(res, 'Giphy API error');
